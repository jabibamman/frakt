...
use crate::{
    types::{fractal_config_path::FractalConfigPath, fractal_descriptor::FractalDescriptor},
    utils::filesystem::project_root,
};
...
pub fn load_fractal_configuration(
    config_path: FractalConfigPath,
) -> Result<FractalDescriptor, io::Error> {
...
mod config_test {
    ...
    use crate::{
        config::load_fractal_configuration,
        types::{
            complex::Complex,
            fractal_config_path::FractalConfigPath,
            fractal_descriptor::{FractalDescriptor, JuliaDescriptor, NewtonRaphsonZ4Descriptor},
        },
    };
    ...
    fn test_load_valid_fractal_configuration() -> Result<(), String> {
        ...
        let loaded_descriptor =
            load_fractal_configuration(config_path).map_err(|e| e.to_string())?;
    ...
    fn test_load_valid_fractal_configuration_with_empty_values() {
        ...
        assert_eq!(
            config_path.to_path(),
            PathBuf::from("config/newton_raphson_z4.toml")
        );

        let loaded_descriptor =
            load_fractal_configuration(config_path).expect("Failed to load fractal configuration");
