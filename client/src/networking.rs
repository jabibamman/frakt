use std::{net::{TcpStream, ToSocketAddrs}, io, time::Duration};

use cli::parser::CliClientArgs;
use image::{ImageBuffer, Rgb};
use log::{error, info, debug};
use server::services::{write::write, reader::get_response};
use shared::{types::{messages::FragmentTask, filesystem::FileExtension, error::FractalError}, utils::{fragment_task_impl::FragmentTaskOperation, filesystem::{get_dir_path_buf, get_file_path, get_extension_str}}};
...
pub fn connect_to_server(cli_args: &CliClientArgs) -> Result<TcpStream, FractalError> {
    ...
    let addrs = address.to_socket_addrs()?.next()
        .ok_or_else(|| FractalError::ConnectionError("Unable to resolve address".to_string()))?;

    let timeout_duration = Duration::from_secs(1); 
    TcpStream::connect_timeout(&addrs, timeout_duration)
        .map_err(FractalError::Io)
}
...
pub fn receive_fragment_task(stream: &mut TcpStream) -> Result<Option<FragmentTask>, FractalError> {
    ...
    match FragmentTask::deserialize(&response) {
        ...
        Err(e) => {
            ...
            Err(FractalError::TaskNotSet(format!("Deserialization error: {}", e)))
        }
    }
}
...
/// 
pub fn save_fractal_image(img: ImageBuffer<Rgb<u8>, Vec<u8>>, img_path: &str) -> Result<(), FractalError> {
...
/// 
pub fn process_fragment_task(task: FragmentTask, open_after_save: bool) -> Result<(), FractalError> {
